...
const MAX_VISIBLE_RANGE_SLOTS = 4; // New Constant
...
export class CalendarView extends ItemView {
    ...
    getDisplayText(): string { return `Year Calendar - ${this.plugin.settings.year}`; }
    ...
    async onClose() { this.clearDayNumberEngagement(); }
    async refresh() { this.leaf.updateHeader(); await this.renderCalendar(); }
    ...
    async renderCalendar() {
        ...
        const holidaysByDate = await this.plugin.holidayService.getAggregatedHolidays(year);
        ...
        for (const file of allFiles) {
            ...
            let explicitColor: string | undefined = fm.color ? fm.color.toString() : undefined;
            let defaultColorFromTag: string | undefined = undefined;
            let defaultSymbolFromTag: string | undefined = undefined;

            if (fm.date) { const mDate = moment(fm.date.toString(), "YYYY-MM-DD", true); if (mDate.isValid()) { validDate = mDate.format("YYYY-MM-DD"); hasDate = true; } }
            if (fm.dateStart && fm.dateEnd) { const mStart = moment(fm.dateStart.toString(), "YYYY-MM-DD", true); const mEnd = moment(fm.dateEnd.toString(), "YYYY-MM-DD", true); if (mStart.isValid() && mEnd.isValid()) { validDateStart = mStart.format("YYYY-MM-DD"); validDateEnd = mEnd.format("YYYY-MM-DD"); hasDate = true; } }
            
            if (!explicitColor && fm.tags) {
                let rawTags: any[] = Array.isArray(fm.tags) ? fm.tags : String(fm.tags).split(',').map(t => t.trim());
                const noteTags = rawTags.map(tag => String(tag).trim()).filter(t => t).map(tag => (tag.startsWith('#') ? tag : `#${tag}`));
                for (const tag of noteTags) {
                    const appearance = this.plugin.settings.tagAppearance[tag];
                    if (appearance) {
                        if (!defaultColorFromTag) { defaultColorFromTag = appearance.color; }
                        if (!defaultSymbolFromTag && appearance.symbol) { defaultSymbolFromTag = appearance.symbol; }
                        if (defaultColorFromTag && defaultSymbolFromTag) break;
                    }
                }
            }

            if (hasDate) { pagesData.push({ path: file.path, date: validDate, dateStart: validDateStart, dateEnd: validDateEnd, name: file.basename, color: explicitColor, defaultColorFromTag: defaultColorFromTag, defaultSymbolFromTag: defaultSymbolFromTag }); }
            if (fm.birthday && (!hasBirthdayFolderSetting || file.path.toLowerCase().startsWith(birthdayFolder))) {
                ...
                if (mBday.isValid()) { birthdayData.push({ path: file.path, birthday: mBday.format("YYYY-MM-DD"), name: file.basename, color: explicitColor, defaultColorFromTag: defaultColorFromTag }); }
            }
        }
        ...
        while (currentWeek.isBefore(endDate)) {
            ...
            weekRow.createEl('td', { cls: 'week-number', text: currentWeek.isoWeek().toString() });

            // --- Weekly Slotting Logic ---
            ...
            for (let i = 0; i < 7; i++) {
                ...
                cellClasses.push(dayMoment.month() % 2 === 1 ? 'odd-month' : 'even-month');
                ...
                if (isHoliday) {
                    ...
                    const holidayColorVar = holidaysOnDay[0].color || 'var(--color-red-tint)';
                ...
                matchingNotes.forEach(note => {
                    const dot = dotAreaDiv.createSpan({ cls: 'dot note-dot', text: note.defaultSymbolFromTag || '●' });
                    dot.title = note.name;
                    dot.style.color = note.color || note.defaultColorFromTag || this.plugin.settings.defaultDotColor;
                });
                if (matchingBirthdays.length > 0) {
                    ...
                    dot.style.color = matchingBirthdays[0].color || matchingBirthdays[0].defaultColorFromTag || this.plugin.settings.defaultBirthdayColor;
                }
                
                // Create empty slots and then fill them
                ...
                matchingRanges.forEach(p => {
                    ...
                    if (slot !== undefined) {
                        ...
                        if (slotEl) {
                            ...
                            bar.style.backgroundColor = p.color || p.defaultColorFromTag || this.plugin.settings.defaultBarColor;
                ...
                if (isHoliday) { expandedHTML += `<ul>${holidaysOnDay.map(h => `<li>${h.name}</li>`).join("")}</ul>`; }
                if (matchingBirthdays.length > 0) { expandedHTML += `<ul>${matchingBirthdays.map(b => `<li><a class="internal-link" data-href="${b.path}" href="${b.path}">${b.name}</a></li>`).join("")}</ul>`; }
                if (matchingNotes.length > 0) { expandedHTML += `<ul>${matchingNotes.map(p => `<li><a class="internal-link" data-href="${p.path}" href="${p.path}">${p.name}</a></li>`).join("")}</ul>`; }
                if (matchingRanges.length > 0) { expandedHTML += `<ul>${matchingRanges.map(p => `<li><a class="internal-link" data-href="${p.path}" href="${p.path}">${p.name}</a></li>`).join("")}</ul>`; }
                if (!isHoliday && !matchingBirthdays.length && !matchingNotes.length && !matchingRanges.length) { expandedHTML += `<p>No events.</p>`; }
                expandedHTML += `</div>`;
                cell.dataset.cellContent = expandedHTML;
            }
            currentWeek.add(7, 'days'); // Correct weekly increment
        }
    }
    ...
    handleClick(event: MouseEvent) {
        ...
        if (dayNumberEl) { this.handleDayNumberClick(event, dayNumberEl); return; }
        const closeButton = target.closest('.close-button');
        if (closeButton) { const expandedRow = closeButton.closest('tr.expanded-row'); if (expandedRow) { this.closeExpandedRow(expandedRow); } return; }
        const internalLink = target.closest('a.internal-link');
        if (internalLink) { event.preventDefault(); const path = internalLink.dataset.href; if (path) { this.app.workspace.openLinkText(path, '', event.ctrlKey || event.metaKey); } return; }
        const cellEl = target.closest('td.calendar-cell');
        if (cellEl) { this.handleCellClick(cellEl); }
    }
    ...
    handleDayNumberClick(event: MouseEvent, dayNumberEl: Element) {
        ...
        if (!cellEl || !cellEl.dataset.date) { this.clearDayNumberEngagement(); return; }
        const dateMoment = moment(cellEl.dataset.date);
        if (isCmdClick) {
            if (this.startRangeDate) {
                const endRangeDate = dateMoment;
                const finalStartDate = this.startRangeDate.isBefore(endRangeDate) ? this.startRangeDate : endRangeDate;
                const finalEndDate = this.startRangeDate.isBefore(endRangeDate) ? endRangeDate : this.startRangeDate;
                this.createRangeNote(finalStartDate, finalEndDate);
                this.clearDayNumberEngagement();
            } else { new Notice("Click a start date first."); }
        } else {
    ...
    handleCellClick(cellEl: HTMLElement) {
        ...
        if (existingExpanded) {
            const previouslyExpandedCell = tbody.querySelector("td.calendar-cell.expanded");
        ...
        if (!wasThisCellExpanded) {
            ...
            const expandedCell = expandedRow.createEl("td", { attr: { colspan: "8" } });
    ...
    clearDayNumberEngagement() {
        if (this.engagedStartRangeEl) { this.engagedStartRangeEl.removeClass('range-start-engaged'); this.engagedStartRangeEl = null; }
